...
function updateTodo(id:number, done:boolean): void {
  editingId = id;
  render();
  ...
  onOutsideClick = function (e: MouseEvent): void {
...
function saveEdit(id:number, newTextRaw: string): void {
...
function isTodo(value: unknown): value is Todo {
  ...
  const maybeId = (value as { [key: string]: unknown }).id; 
  const maybeText = (value as { [key: string]: unknown }).text;
  const maybeDone = (value as { [key: string]: unknown }).done;
